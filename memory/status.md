...
### Done
- Verification only, no code changes: `python3 -m pytest -q` (167 passed, including tasks 5-7's new/updated cases: `test_no_test_sh_skips_without_running_anything`, `test_real_project_agnostic_test_sh_writes_baseline_and_commits`, `test_same_branch_empty_diff_passes`), `ruff check .` (all checks passed), `pyright` (0 errors/warnings/informations) all clean; ran `sh sensors/test.sh` for real in the repo root — passed end-to-end with the `--cov=.` + `.coveragerc` combination (diff-cover reports 100% diff coverage, `_coverage_floor.py` correctly reports "no baseline yet, skipping" since none exists in this worktree yet); grepped the repo for `--cov=runner`/`--cov=bootstrap`/`--cov=cli` and confirmed zero remaining occurrences outside `memory/status.md`'s historical log entries.
- Flagging as an aside: this same plan's tasks 1-4 and 6-8 (only task 5 did, per its own log entry above) never appended a `memory/status.md` entry themselves — confirmed via `git show --stat` on each of their commits, none touch this file. That's a gap in those tasks, not in this verification pass; not fixing it retroactively here since it's out of this task's scope, just surfacing it since this pass was well placed to notice it. It also means `run_loop()`'s own `_main_checkout_dirty_paths`/status-hash-unchanged warning (`runner/loop.py`) would have fired on each of those task runs.
...
## 2026-08-17 — Worker agent plan task 2: wire Worker through `run_subagent`

### Done
- In `runner/loop.py`: added `WORKER_AGENT = "worker"` next to `PLANNER_AGENT`/`REVIEWER_AGENT`; removed the now-unused `WORKER_STATIC_INSTRUCTIONS` constant; changed the fresh-task call site in `_run_one_task` from a manually-prefixed `driver.run(worker_prompt, ...)` to `driver.run_subagent(WORKER_AGENT, task_prompt, context_files=task_context, cwd=handle.path)` — `run_subagent` now injects `agents/worker.md`'s body automatically. Corrective call sites (`SENSOR_CORRECTIVE_INSTRUCTIONS`, `CODE_HEALTH_CORRECTIVE_INSTRUCTIONS`, `REVIEW_CORRECTIVE_INSTRUCTIONS`) untouched, still call `driver.run()` directly.
- In `runner/test_loop.py`: renamed the `_plan_then_approve` helper to `_route_subagent(worker=None)` — a side_effect factory dispatching on `agent_name` (`REVIEWER_AGENT` → approved, `WORKER_AGENT` → delegates to the optional `worker` callback or a plain `_ok()`, else → `PLAN_READY_SIGNAL`), since the worker now shares the `run_subagent` mock with the planner and reviewer. Updated every test that previously split its mocking between `driver.run_subagent` (planner/reviewer) and `driver.run` (worker) to route the worker call through the same `run_subagent` mock instead; `driver.run` mocking is now reserved for corrective retries only, and several tests needed a `driver.run.return_value = _ok()` added back for their expected corrective calls (sensor-fix, code-health-fix, review-corrective) since those still hit `driver.run` and a bare `MagicMock()` fails the retry helpers' `.exit_code != 0` check. Removed the `WORKER_STATIC_INSTRUCTIONS` import and `TestWorkerStaticInstructions` class (constant no longer exists); added `WORKER_AGENT` to imports. Added `test_fresh_task_invokes_run_subagent_with_worker_agent` asserting the fresh-task call uses `driver.run_subagent(WORKER_AGENT, ...)`, never `driver.run`, and that the prompt lacks the old static-instructions preamble text. Also dropped the now-unused `import re` (only used by the removed test class). 246 tests passing repo-wide, `ruff check .` clean.

**Run metrics:** 32 driver call(s), $24.5455, session 19ef388c-d8ec-491d-9fa2-73a9ab88ad73

## 2026-08-21 — Structured-logging plan task 1: module-level logger + `_log_handlers()` helper

### Done
- Added `logger = logging.getLogger("agent_loop")` (level `DEBUG`) to `runner/loop.py`, module top after imports/before constants. Added `_log_handlers(log_path: Path)` context manager (near the existing `# ── Helpers` section, before `_file_hash`): attaches a `StreamHandler` (INFO, `Formatter("%(message)s")`, matching current print output) and a `FileHandler(log_path)` (DEBUG, `Formatter("%(asctime)s %(message)s")`) to `logger`, yields, removes both in `finally` so repeated `run_loop()` calls don't accumulate handlers. Not yet wired into `run_loop()` or used to replace any `print()` calls (later tasks). 135 tests passing, `ruff check .` clean.

## 2026-08-21 — Structured-logging plan task 2: wire `_log_handlers` into `run_loop()`

### Done
- In `run_loop()`: after `run_timestamp` is computed, added `logs_dir = project_root / "logs"` (`mkdir(exist_ok=True)`) and `log_path = logs_dir / f"run-{run_timestamp}.log"`; wrapped the rest of the function body (from `_generate_plan` through the final `return 0`) in `with _log_handlers(log_path):`. The `_implement_tasks` unpack still assigns the plain `task_narratives` name — it's used (passed to `_build_narrative`), not dead.
- `_finalize_run()`: replaced its `task_narratives: list[dict]` and `run_timestamp: str` params with a single `narrative_path: Path`. The narrative machinery itself is untouched — `_build_narrative`/`_write_narrative` are still called with `task_narratives`/`run_timestamp`, just relocated from inside `_finalize_run()` to `run_loop()` immediately before the `_finalize_run()` call, since `_finalize_run()` no longer needs the raw inputs. `_append_narrative_outcome()` and `_offer_merge()` are unchanged, both still keyed off `narrative_path`. This keeps the `.md` narrative file and the new `.log` file temporarily redundant on purpose — task 5 removes the narrative machinery once task 3 (print→logger conversion) lands. 138 tests passing, no breakage, `ruff check .` clean.

## 2026-08-21 — Structured-logging plan task 3: convert all `print()` calls to logger calls

### Done
- Mechanically converted all 59 `print(...)` call sites in `runner/loop.py` to `logger.error(...)` (messages starting with `[error]`), `logger.warning(...)` (`[warning]`), or `logger.info(...)` (everything else) — same message text and f-string content throughout, including the two `header`-variable indirections in `_stop_and_preserve_branch`/`_handle_sensor_failure`/`_handle_worker_failure` (both always `[error]`-prefixed at their call sites). Removed all 25 `sys.stdout.flush()` calls that followed print statements, and removed the now-unused `import sys` (confirmed no other `sys.` references remained). `ruff check .` clean.
- Known fallout, deferred to task 7 per plan.md: `runner/test_loop.py`'s `capsys`/`patch("builtins.print")`-based assertions (21 tests) now fail since output goes through the module logger instead of `print()` — expected, task 7 migrates those to `caplog`.

## 2026-08-21 — Structured-logging plan task 4: debug-level review critique log

### Done
- Added `logger.debug("Task review critique:\n%s", critique)` in `_run_single_review()` (`runner/loop.py`), right after unpacking `approved, critique = _review_verdict(review_result.text)`, then returning the tuple explicitly — additive only, no existing `print()`/`logger` call touched. Per plan.md's own task text, skipped the separate per-task cost debug line: the cost is already emitted at INFO level by `_print_task_metrics`'s `[metrics]` line, so a duplicate DEBUG-level line would add a redundant grep target with no new information. 253 tests passing, `ruff check runner/loop.py` clean.

## 2026-08-21 — Structured-logging plan task 5: remove narrative-assembly machinery

### Done
- Deleted `_build_narrative()`, `_write_narrative()`, and `_append_narrative_outcome()` from `runner/loop.py`. Simplified `_run_task_checks()` to return `(code_health_findings: list[str] | None, failures)` instead of a `narrative_partial` dict — the dropped `review_approved`/`review_reasoning`/`sensor_retries`/`review_retries`/`code_health_retries` fields had no consumer besides `_build_narrative`; `review_critiques` (still needed by `_offer_merge`) is mutated in place by `_run_review_with_retry` regardless. `_run_one_task()` now returns `code_health_findings` directly (no more `narrative` dict). `_implement_tasks()` drops the `task_narratives` accumulator and its slot in the return tuple (now `(exit_code, handle, review_critiques, code_health_issues)`).
- `run_loop()`: removed the `_build_narrative`/`_write_narrative` call site entirely; `_finalize_run()` is now called with `log_path` (already computed in task 2) in the `narrative_path` parameter slot instead of a separately-written `.md` file — the `.log` file is now the only per-run artifact. `_finalize_run()`'s outcome-append is inlined (`with narrative_path.open("a") as f: f.write(...)`) in place of the deleted `_append_narrative_outcome()`, same behavior, no new helper name introduced (left for task 6, which owns the `_append_log_outcome` rename and the ordering nuance of interleaving a raw file-append with the still-active logging `FileHandler` mid-run).
- `runner/test_loop.py`: removed `TestBuildNarrative`, `TestWriteNarrative`, `TestAppendNarrativeOutcome`, and `TestRunLoopNarrative` (all tested behavior that no longer exists); removed the `_read_narrative` helper and the now-dead narrative-content assertions in three `TestRunLoopReviewRetry` tests (kept their call-count/caplog assertions, which still cover the retry behavior). Updated `TestOfferMerge`'s narrative-path test to check the `.log` file and its inlined outcome-append instead of mocking `_append_narrative_outcome`. 131 of 242 repo tests are in `test_loop.py` (down from 142 — narrative-specific tests removed outright, not converted); full suite green, `ruff check .` and `lsp.sh` clean, 100% diff coverage.

## 2026-08-21 — Structured-logging plan task 6: rename `_finalize_run`'s log parameter and extract the outcome-append helper

### Done
- Renamed `_finalize_run()`'s third parameter from `narrative_path: Path` to `log_path: Path` — it's held the run's `.log` file since task 5, and the old name was a leftover from the deleted narrative machinery. Extracted the inline `with narrative_path.open("a") as f: f.write(...)` into a named `_append_log_outcome(path: Path, outcome: str) -> None` (same body, same behavior — appends `\n## Outcome\n{outcome}\n`), called as `_append_log_outcome(log_path, outcome)`. Left `_offer_merge()`'s `narrative_path` parameter name untouched per plan.md's own note — it's a generic "path to open in the editor pane" and isn't specific to the narrative format. 242 tests passing, `ruff check runner/loop.py` clean.

## 2026-08-21 — Structured-logging plan task 7: finish migrating `test_loop.py` off print-capture

### Done
- Most of `test_loop.py` was already caplog-based from tasks 5–6; this task cleaned up the remaining print-era leftovers. Removed 25 now-inert `patch("builtins.print")` context managers (the logger's real handlers make them unnecessary — output goes through `caplog`/the file handler instead, not silenced print). Converted the one remaining `mock_print.call_args_list` assertion (`TestRunLoopCodeHealth::test_clean_on_first_check_commits_without_finding_message`) and the one remaining `capsys.readouterr().out` assertion (`TestRunLoopMainCheckoutLeak::test_no_leak_prints_no_warning`) to `caplog.text`. Dropped two unused `capsys` fixture params that had no reference in their test bodies.
- Added three tests to `TestLogHandlers` per plan.md: `test_console_level_gates_debug_but_not_info` (caplog at INFO vs DEBUG around a `loop_logger.debug()` call), `test_file_handler_receives_debug_with_timestamp` (reads the `.log` file back and checks the debug line got a timestamp prefix, not just the bare message), and `test_repeated_run_loop_calls_do_not_accumulate_handlers` (calls `run_loop()` twice via a gate that rejects immediately, asserts `len(loop_logger.handlers) == 0` before/after each call).
- Fixed a bug in my own bulk-removal script along the way: `prev.rstrip()[:-2]` only strips 2 of the 3 trailing chars in `", \"`, leaving a stray `,:` on 25 lines — caught by `ast.parse` before running tests, fixed with a follow-up `sed -E 's/,:$/:/'`. 134/245 repo tests in `test_loop.py`; full suite green, `ruff check .` clean, 100% diff coverage, whole-repo coverage 95.70% (up from 94.47% baseline).
