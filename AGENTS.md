...
- Worker runs inside a git worktree (separate directory, same repo). Context files are passed as absolute paths. Per-task status.md entries are written into the worktree and travel with that task's own commit (by design — if the branch is abandoned, the narrative entry is abandoned too). The harness's own run-level summary (metrics, plan-rejected) is written directly to the main checkout after the run finishes and auto-committed immediately, so it never sits as dirty state.
